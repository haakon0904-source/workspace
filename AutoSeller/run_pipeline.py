...
def _load_pw():
    ...
    naver_client_id = next((l.split(":",1)[1].strip() for l in lines if l.startswith("Naver Client ID")), "")
...
CONFIG = {
    # 네이버 쇼핑 인사이트
    "naver_client_id": _naver_id,
    "naver_client_secret": _naver_secret,
    "trend_top_n": 10,    # 상위 몇 개 키워드 사용
    "trend_weeks": 4,     # 비교 기간 (주)
...
def main():
    ...
    # Step 1: 트렌딩 키워드 발굴
    print("\n[Step 1] 네이버 쇼핑 인사이트 트렌딩 키워드 분석")
    if CONFIG.get("naver_client_id"):
        from pipeline.step1_trending_keywords import SEED_KEYWORDS
        trending = step1_trending_keywords.run(CONFIG, SEED_KEYWORDS)
        # KEYWORD_CATEGORIES에 없는 키워드는 기본 카테고리로 추가
        for kw in trending:
        ...
        print("[Step 1] 네이버 API 키 미설정 → 수동 키워드 사용")
        keywords = KEYWORDS

    # Step 2: 변형어 확장
    if CONFIG.get("naver_client_id"):
        print("\n[Step 2] 트렌딩 키워드 롱테일 변형어 생성 + 검색량 검증")
        keywords = step2_keyword_variations.run(keywords, CONFIG)
        # KEYWORD_CATEGORIES에 없는 변형어 키워드 기본값 추가
        for kw in keywords:
            if kw not in KEYWORD_CATEGORIES:
                KEYWORD_CATEGORIES[kw] = {
                    "display_category": CONFIG.get("coupang_display_category", 69884),
        ...
        print("\n[Step 2] 네이버 API 키 미설정 → 변형어 생성 생략")
