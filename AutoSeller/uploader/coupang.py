...
def fetch_all_products(config: dict) -> list:
    """
    쿠팡에 등록된 전체 상품 목록 조회.
    목록 API에 items가 없으므로 상세 조회로 externalVendorSku 매핑.
    Returns: list of {seller_product_id, coupang_status, item_no}
    """
    vendor_id = config["coupang_vendor_id"]
    summaries = []
    for status in ("APPROVED", "DRAFT", "UNDER_REVIEW"):
        ...
        data = _request("GET", path, config).json()
        if data.get("code") == "SUCCESS":
    ...
    result = []
    for s in summaries:
        pid = s["seller_product_id"]
        detail = _request(
            "GET",
            f"/v2/providers/seller_api/apis/api/v1/marketplace/seller-products/{pid}",
            config,
        ).json()
        items = detail.get("data", {}).get("items", [])
        sku = items[0].get("externalVendorSku", "") if items else ""
        result.append({
            "seller_product_id": str(pid),
            "coupang_status": s["coupang_status"],
            "item_no": sku,
        })
        time.sleep(0.2)
    return result
