...
- [x] 페이지네이션: `&pg=N` 파라미터, `#lPage ol li[target]` 최대값으로 총 페이지 파악, 10페이지 502개 수집
- [x] 상품 상세 크롤링: `#lThumbImg`(대표이미지), `.lItemPrice`(가격), `.lInfoItemContent`(최소구매수량), `.lInfoDeli`(배송)
  - `networkidle` → `domcontentloaded` + `wait_for_selector` 로 타임아웃 해소

- [x] `pipeline/step3_product_search.py` 정리 완료
